...
BEGIN_MESSAGE_MAP(CLicensePlateRecognitionDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_HOGSVM, &CLicensePlateRecognitionDlg::OnBnClickedHogsvm)
	ON_BN_CLICKED(IDC_TEMP, &CLicensePlateRecognitionDlg::OnBnClickedTemp)
END_MESSAGE_MAP()

//文件路径
//filesearch 函数用于递归搜索指定路径下的所有文件和子目录，并将文件路径存储在 filepathnames 向量中。
...
//垂直投影
//函数用于进行垂直投影，并根据投影结果分割字符。
//具体步骤包括反转二值图像、统计每列白色像素个数、绘制垂直投影直方图、分割字符。
//分割后的字符存储在 roiList 中。
...
//在窗口显示视频
//DrawcvMat 函数用于在窗口中显示图像。
//调整图像大小以适应控件，然后使用 StretchDIBits 函数将图像绘制在窗口上。
...
//显示到屏幕上
//Display 函数用于在界面上显示处理后的图像以及对图像中的车牌信息的预测结果。
//分别在不同的控件上显示处理后的图像，如车牌区域、省份区域、每个字符区域。
//进行车牌信息的预测，包括省份和字符的预测。
//将预测结果更新到界面。
...
vector<Rect> preprocess(Mat src, Mat& dst, vector <Mat>& roi)
{
	...
	for (size_t i = 0; i < contours.size(); i++) // iterate through each contour.
	{
		...
		if (area > largest_area)// 判断当前轮廓面积是否大于最大面积
		{
			// 更新第二大轮廓信息
	...
	if (second_area > 1200 && (second_rect.width / second_rect.height) > 1.5)
		...
		// 定义矩形的四个顶点
			...
			if (second_bounding_rect.angle < -70)//如果矩形的旋转角度小于-70度，说明这是一个倾斜的矩形，这里通过绘制红色线段来显示出原始矩形的倾斜状态。
				for (int i = 0; i < 4; i++)
				{
...
void CLicensePlateRecognitionDlg::OnBnClickedHogsvm()
{
	// TODO: 在此添加控件通知处理程序代码
}


void CLicensePlateRecognitionDlg::OnBnClickedTemp()
{
	// TODO: 在此添加控件通知处理程序代码
}
