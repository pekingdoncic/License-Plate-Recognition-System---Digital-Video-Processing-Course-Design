...
//filesearch 函数通过递归方式搜索指定路径下的所有文件，将文件路径存储在 filepathnames 向量中。
...
//dealHOG 函数接收一个图像（inMat），使用HOG（Histogram of Oriented Gradients）算法计算图像的特征向量，返回一个包含特征向量的Mat。
...
//dealLBP 函数对输入图像进行Local Binary Pattern（LBP）处理，返回处理后的图像。
...
// 裁剪
//remove_border 函数根据图像中的边缘白色像素，裁剪掉图像四周的空白边缘。
...
//distance_c 函数计算两个二值化图像之间的距离，用于后续模型训练中的样本匹配。
...
//PlateRecognizer 类的构造函数和
//train 函数接收图像数据和标签进行模型训练。
...
// init 函数用于初始化模型，包括选择特征模型（HOG或LBP）和设置SVM模型参数。
...
//savemodel 函数保存训练好的模型到指定路径。
void PlateRecognizer::savemodel(string path)
{
	model->save(path);
}

//readmodel 函数根据路径读取已保存的模型，或者从指定路径中读取样本数据。
...
//predict 函数接收输入图像，使用训练好的模型进行车牌字符的预测。结果可以是字符标签或者数字标签。
...
//predict 函数中通过字符匹配的方式，使用模板匹配（OpenCV的 matchTemplate 函数）来获取最匹配的字符。
